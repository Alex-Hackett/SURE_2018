...
class Kinetic_Monte_Carlo:
    def __init__(self, number_sites, number_steps, temp):
        ...
        self.zero_array = []
        self.time_array = []
        self.gamma_k = 1/(50e-12)
    def initialize_system(self):
        for i in range(self.num_sites):
            self.site[i] = choice([-1,1])
            self.update_rates()
    ...
    def update_rates(self):
        ...
        self.rate_k_to_0 +=  self.gamma_k * k_count
        ...
        for i in range(self.num_sites):
            if self.site[i] < 0:
                    self.rate[i] = self.rate_0_to_k/hbar
            elif self.site[i] > 0:
                    self.rate[i] = self.rate_k_to_0 /hbar
...
kmc = Kinetic_Monte_Carlo(100,int(1e4),300)
kmc.simulate()
