...
class kineticMonteCarlo:
    ...
    def update_rates(self):
        k1_k0 = self.k1_to_k0 * (self.k1count) * (1+self.k0count)
        k0_k1 = self.k0_to_k1 * (self.k0count) * (1+self.k1count)
        for i in range(self.num_sites):
            if self.sites[i] == 1:
...
def main():
    ...
    plt.show()
    '''
    filename = 'KMC_test.png'
    plt.savefig(filename)
    plt.close()
    email.email_me('alexjanhackett@gmail.com', 'Portumna1', 'alexjanhackett@gmail.com', 'Image_Test', 'Testing!', filename)
    '''
