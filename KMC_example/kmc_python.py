...
class KMC:
    ...
    def update_rate(self):
        self.RATE_B2A = 5. * ((self.site == 1).sum())**(1/3.) + 0.5
        self.RATE_A2B = 1. * np.sqrt(((self.site == 0).sum())) + 0.5
...
def main():
    ...
    fig, ax = plt.subplots()
    ax.plot(step, kmc.rate_listA2B[0:63], label = r'A $\to$ B Rate')
    ax.plot(step, kmc.rate_listB2A[0:63], label = r'B $\to$ A Rate')
    ax.legend()
    ax.set_xlabel('kMonte Carlo Step')
    ax.set_ylabel(r'Transition Rate ($sec^{-1}$)')
