...
class KMC:
    def __init__(self, sites_num, steps_num):
        self.energy_diff = -1.60217656535e-22#-1.60217656535e-22
        self.RATE_A2B = 0.
        self.RATE_B2A = 0.
        self.temp = 300
        ...
        self.W = 1e7
        self.N_ph_k = math.floor(self.num_sites * (np.exp(((self.energy_diff)/(self.temp * 1.38064852e-23))))**-1)
        self.in_ph = self.N_ph_k
...
def main():
    ...
    for i in range(100000):
        ...
        for i in range(sweeps):
            ...
            '''
            #print("From an Initial Phonon Bath of ",kmc.in_ph)
            #print(kmc.net_phonon_absorb," were absorbed")
            #print("Leaving ",kmc.N_ph_k," Phonons in the bath")
            D_E = kmc.energy_diff * (kmc.in_ph - kmc.N_ph_k)
            delta_temp = D_E / 1.38064852e-23
            #print("Temperature Loss was of Order ", int(delta_temp)," Kelvin")
            #print("Cooling from ",kmc.temp," to ",int(kmc.temp - delta_temp)," Kelvin")
            kmc.temp = kmc.temp - delta_temp
            temp_array.append(kmc.temp)
            time_array.append(kmc.t)
            print("Step ",i," of ",sweeps, "Completed")
            
        times.append(time_array)
        temps.append(temp_array)
    times = np.mean(times, axis=0)
    temps = np.mean(temps, axis=0)
    #temp_data = Table([time_array,temp_array],names = ["Simulation_Time_Seconds","System_Temperature_Kelvin"])
    filename = 'different_e_test.txt'
    #ascii.write(temp_data, filename)
    #print(temp_data)
    fig, ax = plt.subplots()
    ax.plot(times, temps, label = r'GaAs Cooling Curve')
    ax.legend()
    ax.set_xlabel('Time (Seconds)')
    ax.set_ylabel('Microcavity Temperature')
    ax.set_title('Temperature Tracking Acoustic Phonon Scattering Simulation')
    plt.show()
    '''
    print("From an Initial Phonon Bath of ",kmc.in_ph)
    print(np.mean(phonons)," were absorbed")
    print("Leaving ",kmc.N_ph_k," Phonons in the bath")
    D_E = 1.60217656535e-22 * (kmc.in_ph - kmc.N_ph_k)
    delta_temp = D_E / 1.38064852e-23
    print("Temperature Loss was of Order ", int(delta_temp)," Kelvin")
    print("Cooling from ",kmc.temp," to ",int(kmc.temp - delta_temp)," Kelvin")
    kmc.temp = kmc.temp - delta_temp
